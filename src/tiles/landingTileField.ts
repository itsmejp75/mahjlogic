...
export function landingTileStem(def: TileDef): string | null {
  switch (def.cat) {
    case 'suit':
      return `${def.suit}_${def.rank}`
    case 'wind':
      return WIND_STEM[def.wind]
    case 'dragon':
      return def.dragon === 'any' ? 'dragon_red' : `dragon_${def.dragon}`
    case 'flower':
      return `flower_${def.flower}`
    case 'joker':
      return 'joker'
    case 'blank':
      return null
...
export const LANDING_TILE_ASPECT = 4 / 3
...
function buildLandingTileField(
...
): { tiles: readonly LandingFieldTile[]; bounds: { w: number; h: number } } {
  ...
  const tiles: LandingFieldTile[] = raw.map((t) => ({
    def: t.def,
    stem: landingTileStem(t.def) ?? 'joker',
    x: t.x + shiftX,
    y: t.y + shiftY,
    rotate: t.rotate,
    z: t.z,
...
export const LANDING_FIELD_REV = 'table-dump-wallpaper-v5-joker-180'
export const LANDING_TILE_FIELD = buildLandingTileField(0xb8e2c41d, LANDING_FIELD_DEFS)
